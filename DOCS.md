...
**AsuraClient/fca-unofficial** is an unofficial Node.js library for interacting with Facebook Messenger by emulating browser behavior. This library allows you to create chat bots and automate tasks on Facebook Messenger.

## Installation

```bash
npm install AsuraClient/fca-unofficial@latest
```
...
```javascript
const login = require("AsuraClient/fca-unofficial");
...
AsuraClient/fca-unofficial now provides consistent error handling across all modules with custom error classes:

- `FCAError` - Base error class
...
All errors include timestamps, error codes, and detailed context information.

```javascript
const { FCAError, AuthenticationError, NetworkError } = require('AsuraClient/fca-unofficial');

api.sendMessage("Hello", "invalid_thread_id", (err, messageInfo) => {
    if (err instanceof ValidationError) {
        console.error("Validation error:", err.message);
    } else if (err instanceof NetworkError) {
        console.error("Network error:", err.message);
    } else if (err instanceof FCAError) {
...
AsuraClient/fca-unofficial now includes a global error handler middleware that provides:

- Centralized error handling across all modules
- Standardized error codes for easier debugging (e.g., ERR_AUTH_01, ERR_NETWORK_03)
- Automatic retry mechanisms for transient errors
- Error reporting and logging

```javascript
const { handleApiError, ERROR_CODES } = require('AsuraClient/fca-unofficial');

// Example of using global error handler in custom functions
async function customApiFunction() {
    try {
        // Your API logic here
        await someAsyncOperation();
    } catch (error) {
        // Handle error with global middleware
        await handleApiError(error, 'CustomAPI', { 
            retry: true, 
            maxRetries: 3,
            retryDelay: 1000
        });
...
// Access standardized error codes
console.log(ERROR_CODES.AUTH_LOGIN_FAILED); // 'ERR_AUTH_01'
console.log(ERROR_CODES.NETWORK_TIMEOUT);   // 'ERR_NETWORK_01'
```

### 5.3. Handle Checkpoint/Security Check
...
- **GitHub Repository**: https://github.com/AsuraClient/fca-unofficial
- **NPM Package**: AsuraClient/fca-unofficial
...
This is a comprehensive documentation for **AsuraClient/fca-unofficial** API methods. This library is very powerful but should be used carefully to avoid violating Facebook's policies and getting your account banned.
