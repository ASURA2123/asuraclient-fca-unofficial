...
const { NetworkError, AuthenticationError } = require('./errors');
...
function parseAndCheckLogin(ctx, http, retryCount = 0) {
  ...
  const maybeAutoLogin = async (resData) => {
    if (ctx.auto_login) {
      throw new AuthenticationError("Not logged in.", { res: resData });
    }
  ...
  return async (res) => {
    ...
    if (status >= 500 && status < 600) {
      if (retryCount >= 5) {
        throw new NetworkError("Request retry failed. Check the `res` and `statusCode` property on this error.", { 
          statusCode: status, 
          res: res?.data 
        });
      }
    ...
    if (status !== 200) {
      throw new NetworkError("parseAndCheckLogin got status code: " + status + ". Bailing out of trying to parse response.", { 
        statusCode: status, 
        res: res?.data 
      });
    }
    ...
    try {
      ...
      throw new NetworkError("JSON.parse error. Check the `detail` property on this error.", { 
        detail: e, 
        res: resBodyRaw 
      });
    }
    ...
    if (parsed?.error === 1357001) {
      throw new AuthenticationError("Facebook blocked the login");
    }
