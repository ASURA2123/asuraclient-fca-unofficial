...
module.exports = function (defaultFuncs, api, ctx, opts) {
  ...
  function installPostGuard() {
    ...
    logger("postSafe guard installed for defaultFuncs.post", "debug");
    return postSafe;
  }
  ...
  function getSeqIDWrapper() {
    ...
    logger("mqtt getSeqID call", "debug");
    return getSeqIDFactory(defaultFuncs, api, ctx, globalCallback, form)
      .then(() => { logger("mqtt getSeqID done", "debug"); ctx._cycling = false; })
  ...
  return function (callback) {
    class MessageEmitter extends EventEmitter {
      stopListening(callback2) {
        ...
        if (ctx._autoCycleTimer) {
          ...
          logger("mqtt auto-cycle cleared", "debug");
        }

        ctx._ending = true;
        unsubAll(() => endQuietly(() => {
          logger("mqtt stopped", "debug");
          cb();
    ...
    if (conf.cycleMs && conf.cycleMs > 0) {
      ...
      logger(`mqtt auto-cycle enabled ${conf.cycleMs}ms`, "debug");
    } else {
      logger("mqtt auto-cycle disabled", "debug");
    }

    if (!ctx.firstListen || !ctx.lastSeqId) getSeqIDWrapper();
    else {
      logger("mqtt starting listenMqtt", "debug");
